...
export default function ReservationModal({
  ...
  const [overlapsPending, setOverlapsPending] = useState<Reservation[] | null>(
    null,
  );
  ...
  function handleSaveClick() {
    const o = findOverlaps(candidate, reservations);
    if (o.length > 0) {
        ...
        {overlapsPending && (
          <div className="space-y-3">
            ...
            <ul className="space-y-1 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-900">
              {overlapsPending.map((r) => (
                <li key={r.itemId}>
                  &middot;{" "}
                  <span className="font-medium">
                    {formatRangeShort(r.reservedAt, r.durationMinutes)}
                  </span>
                  {r.note ? <span className="text-amber-800"> — {r.note}</span> : null}
                </li>
              ))}
            </ul>
